...
import { GET_USER_VAR, STRAPI_ADD_USER_DATA_AT_PROFILE, UPDATE_USER_VAR } from '../utils/ApiConstants';
...
const Profile = ({ navigation}) => {
  ...
  React.useEffect(() => {
    ...
    console.log('Updating Date:-' + data?.DOB);
  }, [data]);
  ...
  React.useEffect(() => {
    ...
    getShopifyUser(GET_USER_VAR(token))
      .then((result) => {
        ...
        console.log('has Error Label :- ' + someErrorLabel.current);
        console.log('some Error Label' + hasErrorLabel.current);
                  ...
                  <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                    {edit ? (
                      <View style={{ minHeight: Mixins.scaleSize(100), justifyContent: 'space-between' }}>
                        ...
                        <Button onPress={toggleEdit} title="CLOSE" fill="#fafafa" color="#C53437"></Button>
                      </View>
                    ) : (
                      <View style={{ minHeight: Mixins.scaleSize(100), justifyContent: 'space-between' }}>
                        <Button onPress={() => navigation.navigate('Reset', 'Reset Password')} title="RESET PASSWORD" fill="#FFF" color="black" />
                        <Button onPress={handleLogout} title="LOG OUT" fill="#fbfbfb" color="red" />
                      </View>
                    )}
                  </View>
                </View>
